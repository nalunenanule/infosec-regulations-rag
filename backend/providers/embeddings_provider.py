...
from config import DENSE_MODEL_PATH, DENSE_MODEL_NAME, SPARSE_MODEL_PATH, SPARSE_MODEL_NAME
...
class EmbeddingsProvider:
    ...
    def __init__(
        ...
        if not Path(self.dense_model_path).exists():
            ...
            SentenceTransformer(self.dense_model_name).save(self.dense_model_path)
            print("Dense model downloaded.")

    def get_dense_embeddings(self):
        return HuggingFaceEmbeddings(
            model_name=self.dense_model_name,
            cache_folder=self.dense_model_path,
